...
testValueSerialize (void)
{
  ...
  ASSERT_EQUALS_STRING(serializeValue(stringToValue("i0")), "0", "create Value 0");
  ASSERT_EQUALS_STRING(serializeValue(stringToValue("f0.0")), "0.000000", "create Value 0.0");
  ASSERT_EQUALS_STRING(serializeValue(stringToValue("s")), "", "create empty string");
  ASSERT_EQUALS_STRING(serializeValue(stringToValue("bf")), "false", "create Value false");
...
testExpressions (void)
{
  ...
  OP_TRUE(stringToValue("bt"), res, valueEquals, "(Const 10 < Const 20) AND true");
    ...
    // Additional test cases
    MAKE_CONS(l, stringToValue("i5"));
    MAKE_CONS(r, stringToValue("i15"));
    MAKE_BINOP_EXPR(op, l, r, OP_COMP_EQUAL);
    evalExpr(NULL, NULL, op, &res);
    OP_FALSE(stringToValue("bt"), res, valueEquals, "5 == 15");

    MAKE_CONS(l, stringToValue("bt"));
    MAKE_CONS(r, stringToValue("bf"));
    MAKE_BINOP_EXPR(op, l, r, OP_BOOL_OR);
    evalExpr(NULL, NULL, op, &res);
    OP_TRUE(stringToValue("bt"), res, valueEquals, "true OR false");

    // New test cases
    MAKE_CONS(l, stringToValue("f3.5"));
    MAKE_CONS(r, stringToValue("f2.0"));
    MAKE_BINOP_EXPR(op, l, r, OP_COMP_SMALLER);
    evalExpr(NULL, NULL, op, &res);
    OP_FALSE(stringToValue("bt"), res, valueEquals, "3.5 < 2.0");

    MAKE_CONS(l, stringToValue("sHello"));
    MAKE_CONS(r, stringToValue("sWorld"));
    MAKE_BINOP_EXPR(op, l, r, OP_COMP_EQUAL);
    evalExpr(NULL, NULL, op, &res);
    OP_FALSE(stringToValue("bt"), res, valueEquals, "'Hello' == 'World'");
