...
static void test_insertAdditionalTuples(void);
static void test_insertMoreTuples(void);
...
void test_insertAdditionalTuples(void) {
    RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
    TestRecord inserts[] = {
        {16, "pppp", 4},
        {17, "qqqq", 2},
    ...
    RID *rids;
    Schema *schema;
    testName = "test inserting additional tuples";
    schema = testSchema();
    rids = (RID *) malloc(sizeof(RID) * numInserts);
    ...
    // retrieve and compare records from the table
    for(i = 0; i < numInserts; i++) {
        TEST_CHECK(getRecord(table, rids[i], r));
...
void test_insertMoreTuples(void) {
    RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
    TestRecord inserts[] = {
        {21, "uuuu", 4},
        {22, "vvvv", 2},
    ...
    RID *rids;
    Schema *schema;
    testName = "test inserting more tuples";
    schema = testSchema();
    rids = (RID *) malloc(sizeof(RID) * numInserts);
